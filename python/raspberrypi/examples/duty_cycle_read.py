...
def setup():
    ...
    print(
        "Duty Period: %d s  Integration Time: %.1f s"
        % (sensor.getDutyCyclingPeriod(), sensor.getIntegrationTime())
    )
...
def loop():
    ...
    if data is not None:
        msg = (
            "PM1.0: %.1f ug/m3  PM2.5: %.1f ug/m3  PM10: %.1f ug/m3  runtime: %.1f s  runState: %d"
            % (data.PM1, data.PM2_5, data.PM10, data.runtime, data.runState)
        )
